...
class NetWorkWeather: NetworkRequest {
...
//                let flag = params["flag"]
                let userId = params["TokenID"]
//
//                multipartFormData.append((flag?.data(using: String.Encoding.utf8)!)!, withName: "flag")
                multipartFormData.append( (userId?.data(using: String.Encoding.utf8)!)!, withName: "TokenID")
                
                for i in 0..<data.count {
                    multipartFormData.append(data[i], withName: "pictureFile", fileName: name[i], mimeType: "image/png")
                }
        },
            to: urlString,
            headers: headers,
